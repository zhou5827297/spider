...
//            TimeUnit.SECONDS.sleep(60);
    ...
    public void getRankedTravelQuotes() throws InterruptedException {
        ConcurrentLinkedQueue<Future<BigDecimal>> futures = new ConcurrentLinkedQueue<>();
        // 模拟10个计算旅游报价的任务
        for (int i = 1; i <= 20; i++) {
...
//        }
        ...
        for (Future<BigDecimal> future : futures) {
            try {
                totalPriceList.add(future.get(3, TimeUnit.SECONDS));
            } catch (ExecutionException e) {
                // 返回计算失败的原因
                // totalPriceList.add(task.getFailureQuote(e.getCause()));
                totalPriceList.add(BigDecimal.valueOf(-1));
...
//            try {
//                // 10, TimeUnit.SECONDS
//                totalPriceList.add(future.get(5, TimeUnit.SECONDS));
//            } catch (ExecutionException e) {
//                // 返回计算失败的原因
//                // totalPriceList.add(task.getFailureQuote(e.getCause()));
//                totalPriceList.add(BigDecimal.valueOf(-1));
...
//    }
    ...
    public static void main(String[] args) {
        try {
            SubmitThread it = new SubmitThread();
