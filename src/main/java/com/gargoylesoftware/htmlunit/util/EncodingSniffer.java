...
public final class EncodingSniffer {
    ...
    public static Charset sniffHtmlEncoding(final List<NameValuePair> headers, final InputStream content)
        ...
        for (int i = 0; i < 10 ;i++){
            // 头部信息可能会过长，可能没读到，再强制循环读，直到读取到编码为止
            if (encoding == null){
                bytes = readAndPrepend(content, SIZE_OF_HTML_CONTENT_SNIFFED * 4, bytes);
                encoding = sniffEncodingFromMetaTag(bytes);
            } else {
                break;
            }
        }
        // 还取不到编码，加入google的编码检测
        if (encoding == null) {
            String guessEncoding = CharacterUtils.guessEncoding(bytes);
            // 强制把GB2312转化为GBK，兼容繁体和日文
            if ("GB2312".equalsIgnoreCase(guessEncoding)) {
                guessEncoding = "GBK";
            }
            encoding = toCharset(guessEncoding);
        }
        //-----------------------
        return encoding;
    }
    ...
    static Charset sniffEncodingFromHttpHeaders(final List<NameValuePair> headers) {
        ...
        for (final NameValuePair pair : headers) {
            ...
            if ("content-type".equalsIgnoreCase(name)) {
                ...
                if (encoding != null) {
                    // 强制把GB2312转化为GBK，兼容繁体和日文
                    if ("GB2312".equalsIgnoreCase(encoding.name())) {
                        encoding = toCharset("GBK");
                    }
                    break;
                }
            }
        ...
        for (int i = 0; i < bytes.length; i++) {
            if (matches(bytes, i, COMMENT_START)) {
                ...
                for (Attribute att = getAttribute(bytes, i); att != null; att = getAttribute(bytes, i)) {
                    ...
                    if ("charset".equalsIgnoreCase(name) || "content".equalsIgnoreCase(name)) {
